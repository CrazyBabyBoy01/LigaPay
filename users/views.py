...
class UserRegistrationView(SuccessMessageMixin, ContextMixin, CreateView):
    ...
    success_message = (
        'Вы успешно зарегистрированы! На вашу почту отправлено письмо с подтверждением.'
    )
    title = 'Регистрация'
...
class ChangeEmailView(FormView):
    ...
    def form_valid(self, form):
        ...
        transaction.on_commit(
            lambda: send_reset_email.delay(subject, message, recipient_list)
        )

        messages.success(
            self.request, 'Ссылка для подтверждения отправлена на ваш старый email.'
        )
        return redirect(
            'users:email_reset_done'
        )  # перенаправление после успешного запроса на изменение
