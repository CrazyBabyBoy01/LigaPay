...
function init() {
    ...
    camera = new THREE.PerspectiveCamera(27, window.innerWidth / window.innerHeight, 1, 2000); // 45 - дистанция
    ...
    textureLoader.load('/static/deps/chars/assets/bg.png', function (texture) {
        var material = new THREE.MeshPhongMaterial({
            map: texture,
            // side: THREE.DoubleSide,
            depthWrite: true
        });
        ground = new THREE.Mesh(new THREE.CircleBufferGeometry(100, 100), material);
...
function initGUI() {
    ...
    $.getJSON('/static/deps/chars/assets/index2name.json', function (data) {
        // load index2name
        $.each(data, function (key, val) {
            index2name[key] = val;
        });
        // load index2skin
        $.getJSON('/static/deps/chars/assets/index2skin.json', function (data) {
            $.each(data, function (key, val) {
                index2skin[key] = val;
            });
            ...
            // init champion
            options['Персонаж'] = index2name[championIndex.toString()];
            // only need to add once
            gui.add(options, 'Персонаж', names).onChange(function (val) {
                scene.remove(ground);
                groundFlag = false;
                $('#loading').show();
                championIndex = name2index[val];
                window.char_name = val
            ...
            options['Образ'] = index2skin[championIndex.toString()][0];

            skinFolder = gui.add(options, 'Образ', index2skin[championIndex.toString()]).onChange(function (val) {
                scene.remove(ground);
                groundFlag = false;
                $('#loading').show();
    ...
    $.getJSON('/static/deps/chars/assets/name2index.json', function (data) {
        $.each(data, function (key, val) {
            name2index[key] = val;
        });
    });
