...
class StoreSkinsView(ContextMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            logger.warning(
                f"Неавторизованный пользователь попытался оформить заказ с IP {request.META.get('REMOTE_ADDR')}"
            )
        ...
        if form.is_valid():
            ...
            send_reset_email.delay(
                subject=mail_subject, message="", recipient_list=[request.user.email], html_message=html_message
            )
