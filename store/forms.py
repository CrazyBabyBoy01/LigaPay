...
class SkinsOrderForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        try:
            ...
            raise forms.ValidationError('У вас нет кошелька. Обратитесь в поддержку.') from err

        # Работаем с персонажем
        if cleaned_data.get('char_name'):
            json_path = os.path.join(
                settings.BASE_DIR, 'static', 'deps', 'chars', 'assets', 'name2price.json'
            )
        ...
        if cleaned_data.get('skin_name'):
            json_path = os.path.join(
                settings.BASE_DIR, 'static', 'deps', 'chars', 'assets', 'skins2price.json'
            )
            with open(json_path, encoding='utf-8') as file:
                json_data = json.load(file)
